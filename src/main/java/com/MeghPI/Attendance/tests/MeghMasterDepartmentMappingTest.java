...
public class MeghMasterDepartmentMappingTest {
	...
	public void MPI_203_Department_10() throws InterruptedException {
		...
		String captcha = data.get(1);
		 ...
		 deptname = data.get(2) + Pramod.generateRandomAlpha(4);
		 officenameofdept = data.get(3) + initBase.executionRunTime + Pramod.generateRandomAlpha(3);
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	public void MPI_202_Department_09() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	public void MPI_514_Department_mapping_06() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
				"To verify this Select a department, select an office, and assign a Head of Department. Then, map the department with the selected office. After mapping, navigate to the employee's Manage Profile page. Under the Department dropdown, search for the mapped department and ensure that it is displayed. ");

		ArrayList<String> data = initBase.loadExcelData("Master_departmentmapping", currTC,
				"password,captcha,deptname,officename,mailinator,empid,empfirstname,emplastname,empemail,empphoneno,empjoiningdate,reportingto,enrollmentimage,state");
...
//enrolling the User
	...
	public void MPI_510_Department_mapping_02() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
				"Attempt to inactivate a mapped department that is already mapped to an team  Ensure that a proper error message is displayed, preventing the inactivation. ");
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	public void MPI_515_Department_mapping_07() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	@Test(enabled = true, priority = 5, groups = { "Smoke" })
	public void MPI_511_Department_mapping_03() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
				"To verify this:Check the functionality of the search feature by selecting different search options such as Office Name, Employee First Name, Last Name, and Department Name, and ensure the relevant employee records are displayed correctly based on the selected criteria. ");
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	public void MPI_512_Department_mapping_04() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
				"To verify this, disable the mapped department, then navigate to the employee profile. In the department dropdown, ensure that the disabled department is not displayed, provided it is not assigned to any employee.​​​​​​​  ");
...
MeghLoginTest meghlogintest = new MeghLoginTest();
	...
	public void MPI_513_Department_mapping_05() throws InterruptedException {
		String currTC = Thread.currentThread().getStackTrace()[1].getMethodName();
		logResults.createExtentReport(currTC);
		logResults.setScenarioName(
